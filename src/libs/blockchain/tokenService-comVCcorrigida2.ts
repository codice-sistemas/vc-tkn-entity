...
//  const ifaceAll = iface; // seu iface carregado acima com allAbi
  ...
  const processLogs = (logsBatch: Log[]) => {
    for (const log of logsBatch) {
        if (!accountCheck(log)) continue; // mantém filtragem pelo cliente
        ...
        // não faz nada, continuará para fallback
        }

        console.log(parsed)

        if (parsed) {
        // AGORA: pega todos os eventos do cliente, sem filtrar pelo nome
        ...
        try {
            ...
            // checa se algum arg contém o endereço do cliente
            const hasClient = Object.values(argsObj).some((a: any) => {
            if (!a) return false;
            const s = String(a).toLowerCase();
            return s.includes(normalizedHash.replace(/^0x/, "")) ||
                    (smartAccountAddress && s.includes(smartAccountAddress.replace(/^0x/, "").toLowerCase()));
            });
        ...
        if ([fb.sender, fb.IF, fb.tokenType, fb.tokenId].some(v =>
            v && (v.toLowerCase() === normalizedHash || 
            (smartAccountAddress && v.toLowerCase().includes(smartAccountAddress.toLowerCase().slice(2))))
        )) {
